...
correlated therefore, we can expect the FWER to be less than $2\alpha$.
...
In the case outlined in the hint, we will always have 1 false positive (type 1
error), which, implies that when negatively correlated we expect the FWER
to be greater than $2\alpha$.
