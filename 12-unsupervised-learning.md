...
We follow Algorithm 12.1: missing entries are initialized to the column mean,
...
ggplot(df, aes(frac, MSE, color = factor(M), group = M)) +
  geom_line() +
  geom_point() +
  labs(
...
on standardized `Boston`. Beyond that the rank-$M$ approximation starts to
