...
n <- sort(sapply(1:13, function(i) {
  setNames(abs(wilcox.test(x[, i] ~ x$highcrim)$p.value), colnames(x)[i])
}))
n
```
...
```{r}
res <- sapply(1:13, function(max) f(names(n)[1:max]))
(res <- t(res))

ggplot(reshape2::melt(res), aes(Var1, value, col = Var2)) + 
  geom_line() + 
...
```{r}
res <- sapply(1:8, function(max) f(names(n)[1:max], k_vals = 20))
(res <- t(res))
ggplot(reshape2::melt(res), aes(Var1, value, col = Var2)) + 
  geom_line() + 
  xlab("Number of variables") + 
