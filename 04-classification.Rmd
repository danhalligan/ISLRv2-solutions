...
```{r}
ord <- order(sapply(1:13, function(i) {
...
Fit lda, logistic regression, naiveBayes and KNN models (with k = 1..50) for a
...
res <- sapply(1:13, function(max) fit_models(1:max))
(res <- as_tibble(t(res)))
res$n_var <- 1:13
...
```{r}
res <- sapply(1:8, function(max) fit_models(1:max, k_vals = 20))
(res <- as_tibble(t(res)))
res$n_var <- 1:8
pivot_longer(res, cols = !n_var) |>
  ggplot(aes(n_var, value, col = name)) +
...
with 7 predictors also performs well and has an error rate of ~13%.
